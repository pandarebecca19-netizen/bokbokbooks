const SPINE_H = 190;
// mobile spines are shorter so the shelf doesn't feel cramped on a phone
...
// 전자책(format === "ebook")은 쪽수 대신 진행률(%)을 직접 입력하고,
// 그때는 current_page 자체가 0~100 사이의 퍼센트 값이다.
export function readingProgress(book) {
  if (book.format === "ebook") {
    const percent = Math.max(0, Math.min(100, Number(book.current_page) || 0));
    return { current: null, total: null, percent, isPercent: true };
  }
  const total = Number(book.pages) || 0;
  if (!total) return null;
  const current = Math.max(0, Math.min(total, Number(book.current_page) || 0));
  const percent = Math.round((current / total) * 100);
  return { current, total, percent, isPercent: false };
}
...
export function spineWidthForMobile(book) {
  ...
  const titleLen = (book.title || "").length;
  const charsPerColumn = Math.max(1, Math.floor((MOBILE_SPINE_H - 40) / 12));
  const columnsNeeded = Math.max(1, Math.ceil(titleLen / charsPerColumn));
...
function compareBooksBy(sortKey) {
...
// "읽는 중" books always float to the top of the list, sorted by the
// chosen criterion within that group; everything else follows below,
// sorted the same way.
export function sortBooks(books, sortKey) {
  const compare = compareBooksBy(sortKey);
  const reading = books.filter((b) => b.status === "reading").sort(compare);
  const rest = books.filter((b) => b.status !== "reading").sort(compare);
  return [...reading, ...rest];
}

export { SPINE_H, MOBILE_SPINE_H };
